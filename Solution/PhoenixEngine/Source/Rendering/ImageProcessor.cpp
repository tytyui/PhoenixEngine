...
const UInt16 FImageProcessor::MaxWidth = 1 << 14;
const UInt16 FImageProcessor::MaxHeight = 1 << 14;
...
void FImageProcessor::Load(const FLoadParams& LoadParams) {
	...
	Data.Width = static_cast<UInt16>(ImageWidth);
	Data.Height = static_cast<UInt16>(ImageHeight);
...
const UInt8* FImageProcessor::GetPixels() const {
	return Data.Pixels;
}

UInt32 FImageProcessor::GetPixelCount() const {
...
UInt16 FImageProcessor::GetWidth() const {
	return Data.Width;
}

UInt16 FImageProcessor::GetHeight() const {
	return Data.Height;
}
